...
export const get_effects = (
    ...
    // ensure that all rounds are of anchor round's height (or 0)
            ...
            rounds.forEach((r, i) => r.style.height = i < Math.floor(anchor_index) ? 0 : 'auto') // squash invis rounds so that their height didn't affect the area's height
            anchor_index > 0
                && (rounds[Math.floor(anchor_index) - 1].style.height = rounds[Math.floor(anchor_index)].clientHeight + 'px')
