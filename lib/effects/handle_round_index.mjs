...
import { update_buttons } from '../navigation/update_buttons.mjs'
import { update_scrollbar } from '../stable_elements/scrollbar.mjs'

export const handle_round_index = (all_data, get_option, state, stable_elements) => {
    console.log('handle_round_index. Is it called too often on resize?')

    const { the_root_element,
...
// try switch to mobile layout
    let should_be_mobile = false
    if (get_option('useMobileLayout')) {
        should_be_mobile = true
    } else {
        const round = the_root_element.querySelector('.round-wrapper.anchor-round')
        should_be_mobile = get_option('autoSwitchToMobileLayout')
            && round
            && (round.clientWidth > content_horizontal_scroller.clientWidth)
    }
    the_root_element.classList[should_be_mobile ? 'add' : 'remove']('mobile')



    const anchor_index = Math.min(all_data.rounds.length - 1, state.anchor_round_index)
...
// shift matches and round titles horizontally according to anchor index
    ...
    content_area.style.left = -(hor_scroll_px + get_option('connectionLinesWidth')) + 'px'
...
// adjust scroll position to keep the same matches in the middle
    ...
    update_buttons(stable_elements, anchor_index, all_data, get_option)
