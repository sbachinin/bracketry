...
export const create_scrolla = (html_shell, get_option) => {
    ...
    const make_scroll_jump = (deltaY) => {
        ...
        matches_positioner.style.transform = `translateY(${-synthetic_scrollTop}px)`
    ...
    const get_scrollY_ratio = () => {
        ...
        return scrollY_middle_px / scroller.scrollHeight
    }
    ...
    const adjust_offset = (scrollY_middle_ratio) => {
        ...
        if (get_option('resetScrollOnNavigation')) {
            ...
            matches_positioner.style.transform = 'translateY(0)'
            ...
            if (get_option('verticalScrollMode') === 'buttons' || get_option('fullscreen') === true) {
                ...
                matches_positioner.style.transform = `translateY(${-new_offset}px)`
                setTimeout(() => {
                    matches_positioner.style.transition = `transform 0.2s ease-out`
                })
            } else {
