...
// create vert scroll buttons, if they weren't
    ...
    const canvas_vert_margin = options.vertical_scroll_buttons_position === 'In the gutters'
        ? options.vertical_scroll_buttons_clickable_height
        : 0

    const canvas_height = wrapper.clientHeight
        - (options.hide_round_titles ? 0 : get_round_title_height(options))
        - canvas_vert_margin * 2

    main_canvas_el.width = canvas_width
    main_canvas_el.height = options.auto_canvas_height
        ? get_total_rounds_height(options, all_data.rounds[0].matches.length)
        : canvas_height
    ...
    expand_canvas_el.height = canvas_height
    expand_canvas_el.style.top = get_round_title_height(options) + canvas_vert_margin
    expand_canvas_el.style.left = canvas_hor_margin
}
