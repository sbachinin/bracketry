import { get_round_title_height, get_round_height, get_width_per_round } from '../utils/sizes.mjs'
...
export const update_root_elements = (root_elements, options, all_data, state) => {
    ...
    const first_round_height = get_round_height(all_data.rounds[0], matches_scroller.clientHeight, options)
    const anchor_round_height = get_round_height(all_data.rounds[state.anchor_round_index], matches_scroller.clientHeight, options)

    const all_rounds_width = get_width_per_round(options) * all_data.rounds.length
    main_canvas_el.width = all_rounds_width
    ...
    matches_scrollable_area.style.width = all_rounds_width + 'px'
    matches_scrollable_area.style.height = first_round_height + 'px'
// create hor scroll buttons, if they weren't
    const content_is_wider = all_rounds_width > matches_scroller.clientWidth
    ...
    root_elements.hor_buttons?.update_visibility(all_data, options, state.anchor_round_index)
    ...
    if (options.hide_round_titles) {
        ...
        round_titles_canvas_el.width = all_rounds_width
