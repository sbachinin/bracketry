...
export const create_root_elements = (user_wrapper_el, root_id, resize_cb, options) => {
    ...
    const round_titles_html = `<div class="round-titles-wrapper"
        style="
            height: ${options.round_titles_height}px;
            border: none;
    ...
    const matches_content_area_html = `<div class="matches-content-area with-hidden-scrollbar"
        style="
            flex: 1;
            position: relative;
            overflow-y: scroll;
        ...
        "
    >
        <div class="rounds-elements-wrapper" style="position: absolute; top: 0; width: 100%; height: 100%; border: 10px solid blue;"></div>
        <canvas class="lines-canvas" style="border: none; border: 5px solid navy; user-select: none"></canvas>
    </div>`
    ...
    const matches_content_area = the_root_element.querySelector('.matches-content-area')
