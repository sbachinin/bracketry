...
export const create_vertical_scroll_buttons = (
    ...
    const update_visibility = debounce((options, scroll_Y) => {
        up_button.wrapper.classList[scroll_Y <= 0 ? 'add' : 'remove']('disabled')
        up_button.wrapper
            .classList[options.vertical_scroll_triggered_by === 'buttons' ? 'remove' : 'add']
            ('hidden')
        
        down_button.wrapper.classList[scroll_Y >= 100 ? 'add' : 'remove']('disabled')
        down_button.wrapper
            .classList[options.vertical_scroll_triggered_by === 'buttons' ? 'remove' : 'add']
            ('hidden')
    }, 50)
