...
export const get_match_element = (round_id, match_index, all_data, get_option) => {
    const maybe_match_data = all_data.matches.find(m => m.round_id === round_id && m.order === match_index)
...
// maybe use custom match renderer provided by user
    ...
    if (render_match !== null) {
        match_body_element.append(render_match(round_id, match_index, maybe_match_data))
