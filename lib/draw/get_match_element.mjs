...
const get_player_el = (player, get_option) => {
    let nationality = ''
    if (typeof player.nationality === 'string') {
        let nat_getter = get_option('getNationalityHTML')
        if (typeof nat_getter === 'function') {
            let maybe_nationality = nat_getter(player.nationality)
            if (typeof maybe_nationality === 'string') { 
                nationality = maybe_nationality
            }
        } else {
...
const get_side_html = (side, other_side, all_data, get_option) => {
    ...
    if (side) {
        ...
        if (typeof side.contestant_id === 'string') {
            ...
            players_html = get_player_el({ title: side.title }, get_option)
        }
        ...
        if (contestant) {
            entry_status = (typeof contestant.entry_status === 'string') ? contestant.entry_status.trim() : ''
            players_html = contestant.players.map(pl => get_player_el(pl, get_option)).join('')
        }
    }
...
// otherwise: render a contentful match using default layout
    match_body_element.innerHTML += `
        <div class="sides">
            ${get_side_html(maybe_match_data.sides?.[0], maybe_match_data.sides?.[1], all_data, get_option)}
            ${get_side_html(maybe_match_data.sides?.[1], maybe_match_data.sides?.[0], all_data, get_option)}
        </div>
    `
