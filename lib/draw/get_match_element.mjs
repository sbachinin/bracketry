...
const get_side_content = (side, other_side, all_data) => {
    ...
    if (side.contestant_id) {
        let contestant = all_data.contestants?.[side.contestant_id]
    
        if (contestant && all_data.have_entry_statuses) {
        ...
        // TODO layout problem: if side-title is rendered agains a side with entry_status or/and nationality, 
        // it leads to a ugly hor. misalignment
        content += `<div class="side-title">${ side.title }</div>`
    }
...
const get_side_html = (side, other_side, all_data) => `
    <div
        ...
        "
        ${ side?.contestant_id ? ` contestant-id="${ side.contestant_id }"` : '' }
    >
