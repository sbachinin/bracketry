...
// create a shell of a match
    const match_wrapper_element = create_element_from_Html(`
        <div
            class="
                match-wrapper
                ${Array.isArray(maybe_match_data?.sides) && maybe_match_data?.sides.length && maybe_match_data?.isLive === true ? 'live' : ''}
...
// otherwise: render a contentful match using default layout
    // <sides> are rendered for a contentful match anyway because it helps align <match-status>
