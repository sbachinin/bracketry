...
export const get_match_element = (round_index, match_index, all_data, get_option) => {
    const maybe_match_data = all_data.matches.find(m => m.round_index === round_index && m.order === match_index)
...
// create a shell of a match
    const match_wrapper_element = create_element_from_Html(`
        <div
            class="match-wrapper ${ maybe_match_data?.is_live ? 'live' : ''} ${match_index%2 === 0 ? 'even' : 'odd'}"
            ${maybe_match_data?.match_id ? `match-id="${maybe_match_data.match_id}"` : ''}
        >
...
// maybe use custom match renderer provided by user
    ...
    if (render_match !== null) {
        match_body_element.append(render_match(round_index, match_index, maybe_match_data))
