...
const get_player_el = (player) => {
    const flag_img_html = (
        typeof player.flag_url === 'string' && !get_failed_img_srcs().includes(player.flag_url)
        ) ? `<img class="player-flag" src="${player.flag_url}" loading=lazy />`
        : ''

    const nationality = typeof player.nationality_code === 'string' ? player.nationality_code : ''
    
// TODO is .player-wrapper necessary?
    return `<div class="player-wrapper">
        <div class="nationality">${flag_img_html || nationality}</div>
