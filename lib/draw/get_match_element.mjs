...
const get_entry_status_html = (context, all_data, get_option) => {
    ...
    if (typeof contestant.entry_status === 'string') {
        result = contestant.entry_status.trim()
    }

    // if getEntryStatusHTML is a function, it should be called anyway, even if contestants[i].entry_status is missing or invalid
    if (typeof get_option('getEntryStatusHTML') === 'function') {
        try {
            const unreliable_value = get_option('getEntryStatusHTML')(
                contestant.entry_status, context, all_data
            )
...
const get_side_html = (match, side_index, all_data, get_option) => {
    ...
    if (this_side) {
        if (this_side.is_winner) { winner_class = 'winner' }
        if (typeof this_side.contestant_id === 'string') {
            contestant_id = `contestant-id="${this_side.contestant_id}"`
        ...
        if (!this_side.is_serving && !other_side?.is_serving) { hidden_serving_class = 'hidden' }
        if (other_side?.is_serving) { transparent_serving_class = 'transparent' }

        let contestant = all_data.contestants?.[this_side.contestant_id]

        if (contestant) {
            entry_status = get_entry_status_html(
                { matchId: match.id, contestantId: this_side.contestant_id },
                all_data,
                get_option
            )
            ...
            players_html = (contestant.players || []).map(
                (pl, playerIndex) => {
                    return get_player_html(
                        pl,
                        { matchId: match.id, contestantId: this_side.contestant_id, playerIndex },
                        all_data,
                        get_option
                    )
                }
