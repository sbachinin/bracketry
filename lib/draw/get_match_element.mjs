...
const get_player_html = (player, context, all_data, get_option) => {
    ...
    if (typeof get_option('getPlayerTitleHTML') === 'function') {
        try {
            const unreliable_value = get_option('getPlayerTitleHTML')(
                player, context, all_data
            )
            if (typeof unreliable_value === 'string') {
                title = unreliable_value
            } else {
                throw `options.getPlayerTitleHTML must return a string, instead returned: ${unreliable_value}`
            }
        } catch (e) {
            console.warn('Failed to get a string from getPlayerTitleHTML.', e)
        }
    }
