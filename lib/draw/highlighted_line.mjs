...
const find_rightmost_visible_highlighted_match = (rounds, state, options, canvas_el) => {
    let match = null
    let round = null
    ...
    rounds.find((_, r_i) => {
        ...
        if (r.left_X - options.distance_between_rounds - state.scroll_X > canvas_el.width) return false
...
export const draw_highlighted_line = (
    ...
    // highlight_ctx.setLineDash([8, 16])

    const last_highlighted = find_rightmost_visible_highlighted_match(
        rounds, state, options, highlight_ctx.canvas)
    ...
    highlight_ctx.moveTo(
        last_highlighted.round.left_X - state.scroll_X + last_highlighted.round.width
