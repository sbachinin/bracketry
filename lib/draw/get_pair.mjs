...
export const get_pair = (pair_class, match, side0_players_info, side1_players_info) => {
    return `<div class="match-info-pair ${pair_class}" match-id="${match.id}">
        <div
            class="side-info-item ${ match.sides[0]?.result === 'winner' ? 'winner' : ''}"
            contestant-id="${match.sides[0]?.contestant_id || ''}"
        >
            ${
                side0_players_info
                ? `<div class="players-info-wrapper">${side0_players_info}</div>`
                : ''
            }
            ${get_height_maker_for_side(match.sides)}
        </div>
        <div
            class="side-info-item ${ match.sides[1]?.result === 'winner' ? 'winner' : ''}"
            contestant-id="${match.sides[1]?.contestant_id || ''}"
        >
            ${
                side1_players_info
                ? `<div class="players-info-wrapper">${side1_players_info}</div>`
                : ''
            }
            ${get_height_maker_for_side(match.sides)}
        </div>
