...
export const get_expanded_match_props = (round, match, state, options) => {
    ...
    const should_draw_entry_status = !!match.sides.find(s => s.entry_status !== undefined)
    const entry_status_actual_width = (should_draw_entry_status ? options.entry_status_width : 0)
    ...
    const expanded_width = Math.ceil(options.match_padding_left
        + entry_status_actual_width
        + options.nationality_width
        + title_actual_width
        + options.scores_left_margin
        + scores_actual_width
        + options.match_padding_right)

    let expanded_left_X = round.left_X - entry_status_actual_width - options.nationality_width - Math.floor(state.scroll_X)
    expanded_left_X = Math.max(10, expanded_left_X)

    let top_Y = Math.floor(-options.vert_gap_between_opponents / 2 - options.team_title_font_size * 1.4)
    if (options.connection_lines_width % 2 !== 0) top_Y += 0.5
    
    const match_height = Math.ceil(options.vert_gap_between_opponents + options.team_title_font_size * 2.8)
...
//
    ...
    ctx.translate(options.nationality_width + options.team_title_left_margin, 0)

//
    draw_teams_titles({ match, is_expanded: true, options, ctx })
    ctx.translate(title_actual_width + options.scores_left_margin, 0)
