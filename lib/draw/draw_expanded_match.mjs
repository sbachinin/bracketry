...
export const get_expanded_match_props = (round, match, state, options) => {
    ...
    let expanded_left_X = round.left_X - seed_actual_width - Math.floor(state.scroll_X)
    const team_title_left_X = options.match_padding_left + seed_actual_width
    
    const scores_left_X = team_title_left_X + title_actual_width + options.scores_left_margin
    let top_Y = Math.floor(center_scroll_Y - options.vert_gap_between_opponents / 2 - options.team_title_font_size * 1.4)
...
export const draw_expanded_match = ({ match, round, state, ctx, options }) => {
    ...
    ctx.translate(expanded_left_X, 0)
...
//
    if (match.sides.find(s => s.seed_text !== undefined)) {
        draw_seed({ match: match_with_Y, options, ctx }) }
