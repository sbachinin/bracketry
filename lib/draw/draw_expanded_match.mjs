...
const get_expanded_match_props = (match, state, main_canvas_el, options) => {
    ...
    * Math.max(...match.sides.map(s => s.score.length))
    ...
    let expanded_left_X = match.round_left_X - seed_actual_width - Math.floor(state.scroll_X)
...
export const draw_expanded_match = ({ match, round, state, main_canvas_el, options }) => {
    if (match.round_left_X > state.scroll_X + main_canvas_el.width) return
    ...
    const {
        ...
        top_Y,
        match_height
    } = get_expanded_match_props(match_with_Y, state, main_canvas_el, options)
...
//
    if (match.sides.find(s => s.seed_text !== undefined)) {
        draw_seed({ match: match_with_Y, current_X: seed_left_X, options, ctx }) }
