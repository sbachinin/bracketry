...
const get_expanded_match_props = (match, state, main_canvas_el, options) => {
    ...
    * Math.max(...match.sides.map(s => s.score.length))
    ...
    const expanded_left_X = match.round_left_X - seed_actual_width - Math.floor(state.scroll_X)
...
export const draw_expanded_match = ({ match, round, state, main_canvas_el, options }) => {
    ...
    if (center_scroll_Y < -round.height_per_match || center_scroll_Y > main_canvas_el.height + round.height_per_match) return
