...
export const get_expanded_match_props = (round, match, state, options, ctx) => {
    ...
    const entry_status_actual_width = Math.max(...match.sides.map(s => {
        return s.entry_status !== undefined ? ctx.measureText(s.entry_status).width : 0
    }))
    ...
    let top_Y = Math.floor(-VERT_GAP_BTW_OPPONENTS / 2 - options.match_font_size * 1.4)
    if (options.connection_lines_width % 2 !== 0) top_Y += 0.5
    
    const match_height = Math.ceil(VERT_GAP_BTW_OPPONENTS + options.match_font_size * 2.8)
...
export const draw_expanded_match = ({ match, round, state, ctx, options }) => {
    ...
    const {
        ...
        title_actual_width,
        entry_status_actual_width
    } = get_expanded_match_props(round, match, state, options, ctx)
