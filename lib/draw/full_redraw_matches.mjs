...
// shrink the rounds wrapper according to anchor round
    ...
    const leftmost_visible_round_height = rounds_elements_wrapper.firstChild.clientHeight
        / Math.pow(2, Math.floor(store.state.anchor_round_index))
...
// shift matches and round titles horizontally according to anchor index
    ...
    const max_scroll_X = Math.max(0, rounds_elements_wrapper.clientWidth - content_horizontal_scroller.clientWidth)
    const hor_scroll_px = Math.min(
        store.state.anchor_round_index * rounds_elements_wrapper.clientWidth / all_data.rounds.length,
        max_scroll_X
    )
...
// shrink rounds_elements_wrapper according to the distance from 1st round
