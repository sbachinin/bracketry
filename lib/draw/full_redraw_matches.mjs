...
// remember scrollY ratio
    ...
    const scrollY_middle_ratio = scroll_middle_px / matches_vertical_scroller.scrollHeight
...
// create and append round elements with all match info
    ...
    all_data.rounds.forEach((round, round_index) => {
        round_elements.push(
            get_round_element(round, all_data.rounds.length, round_index, store.state, options)
        )
    })
...
// shift matches and round titles horizontally according to anchor index
    ...
    const max_scroll_X = Math.max(0, widest_round_width * all_data.rounds.length - content_horizontal_scroller.clientWidth)
    const hor_scroll_px = Math.min(store.state.anchor_round_index * widest_round_width, max_scroll_X)
...
// update scrollbar visibility
    const content_is_higher = height_with_paddings > matches_vertical_scroller.clientHeight
...
// update hor buttons visibility & activeness
    ...
    const right_is_active = store.state.anchor_round_index < get_max_scroll_round_index(root_elements, all_data)
