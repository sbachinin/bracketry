...
const get_expanded_match_props = (match, state, main_canvas_el, options) => {
    ...
    * Math.max(...match.sides.map(s => s.score.length))
    ...
    if (expanded_left_X + expanded_width + 10 > main_canvas_el.width) {
        expanded_left_X = main_canvas_el.width - expanded_width - 10
    }
    ...
    const top_Y = match.center_scroll_Y - options.vert_gap_between_opponents / 2 - options.team_title_font_size * 1.4
...
export const maybe_draw_expanded_match = ({ match, state, main_canvas_el, options }) => {
    if (!match || typeof match.id === 'undefined') return

    if (match.collapsed_left_X > state.scroll_X + main_canvas_el.width) return
    ...
    const {
        ...
        top_Y,
        match_height
    } = get_expanded_match_props(match, state, main_canvas_el, options)
...
//
    if (match.sides.find(s => s.seed_text !== undefined)) {
        draw_seed({ match, current_X: seed_left_X, options, ctx }) }

//
    draw_teams_titles({
        match,
        current_X: team_title_left_X,
...
//
    draw_scores({ ctx, match, current_X: scores_left_X, highlighted_team_id: state.highlighted_team_id, options })
