...
const get_expanded_match_props = (match, state, canvas_el, options) => {
    ...
    const title_actual_width = Math.max(...match.sides.map(s => ctx.measureText(s.short_title).width))
...
export const maybe_draw_expanded_match = (match, state, canvas_el, options) => {
    ...
    if (match.id === state.previous_expanded_match_id) {
        ctx.globalAlpha = 1 - state.hovered_match_opacity
    }
...
//
    draw_teams_titles(match.sides, team_title_left_X, match.center_Y, options, ctx, true)
