...
const get_expanded_match_props = (match, state, canvas_el, options) => {
    ...
    * Math.max(...match.sides.map(s => s.score.length))
    const expanded_width = match.collapsed_width + options.scores_left_margin + scores_actual_width

    let expanded_left_X = match.collapsed_left_X - state.scroll_X
    ...
    const team_title_left_X = expanded_left_X + options.match_padding_left
