    ...
    Array.from(Array(match_count)).map((_, match_index) => {
        ...
        info_pairs_html += `
            <div class="match-info-pair entry-statuses">
                ${
                    m.sides[0]?.[0]?.entry_status === undefined
                    ? dummy_item
                    : `<div>${m.sides[0]?.entry_status}&#8203;</div>`
                }
                ${
                    m.sides[1]?.[0]?.entry_status === undefined
                    ? dummy_item
            ...
            <div class="match-info-pair nationalities">
                ${get_nationality_item(m.sides[0]?.[0])}
                ${get_nationality_item(m.sides[1]?.[0])}
            </div>

            <div class="match-info-pair team-titles" match-id="${m.id}">
                ${
                    m.sides[0]?.[0]?.title === undefined
                    ? dummy_item
                    : get_title_item(m.sides[0]?.[0], state)
                }
                ${
                    m.sides[1]?.[0]?.title === undefined
                    ? dummy_item
                    : get_title_item(m.sides[1]?.[0], state)
                }
            </div>

            <div class="match-info-pair" style="align-items: center;">
                ${ get_result_mark(m.sides[0]?.[0]?.result, options) }
                ${ get_result_mark(m.sides[1]?.[0]?.result, options) }
            </div>
