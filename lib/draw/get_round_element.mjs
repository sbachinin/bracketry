...
const get_side_content = (side, other_side, all_data) => {
    ...
    if (side.is_winner) {
        content += `<div class="side-info-item winner-mark">
            <svg class="default-winner-svg" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M9 21.035l-9-8.638 2.791-2.87 6.156 5.874 12.21-12.436 2.843 2.817z"/></svg>
        </div>`
    }
...
const get_side_el = (side, other_side, all_data) => {
    return `<div class="side-wrapper ${ other_side?.is_winner ? 'looser' : '' }"
        ${side ? ` contestant-id="${ side.contestant_id }"` : ''}
    >
        <div class="side-body ${side?.is_winner ? 'winner' : ''}">
