...
const get_value_or_dummy_el = (maybe_value) => {
        ...
        <div class="round-wrapper">
                ...
                ${populate_round_column(
                    ...
                    (m) => `
                        <div class="match-info-pair" style="text-align: center;">
                            ${get_value_or_dummy_el(m.sides[0]?.entry_status)}
                            ${get_value_or_dummy_el(m.sides[1]?.entry_status)}
                        </div>
                    `
                )}
            </div>
            <div class="round-column nationalities" style="align-items: center;">
                ${populate_round_column(
                    ...
                    (m) => `
                        <div class="match-info-pair" style="text-align: center;">
                            ${get_value_or_dummy_el(m.sides[0]?.nationality_code)}
                            ${get_value_or_dummy_el(m.sides[1]?.nationality_code)}
                        </div>
                    `
                )}
            </div>
            <div class="round-column team-titles" style="align-items: flex-start;">
                ${populate_round_column(
                    ...
                    (m) => `
                        <div class="match-info-pair">
                            ${get_value_or_dummy_el(m.sides[0]?.title)}
                            ${get_value_or_dummy_el(m.sides[1]?.title)}
                        </div>
                    `
                )}
            </div>
            <div class="round-column result-mark" style="align-items: center;">
                ${populate_round_column(
                    match_count,
                    round.matches,
                    (m) => `
                        <div class="match-info-pair" style="text-align: center;">
                            ${get_value_or_dummy_el(
                                m.sides[0]?.result === 'winner' ? options.winner_mark : m.sides[0]?.result
                            )}
                            ${get_value_or_dummy_el(
                                m.sides[1]?.result === 'winner' ? options.winner_mark : m.sides[1]?.result
                            )}
                        </div>
                    `
                )}
