...
const get_side_content = (side, all_data) => {
    ...
    if (all_data.have_nationalities) {
        content += `<div class="side-info-item nationalities">
            ${
                side.players.map(pl => {
                    if (pl.flag_url && !get_failed_img_srcs().includes(pl.flag_url)) {
                        return `<div><img class="player-flag" src="${pl.flag_url}" loading=lazy /></div>`
                    }
                    if (pl.nationality_code) {
                        return `<div>${ pl.nationality_code }</div>`
                    }
                    return `<div class="player-empty-item"></div>`
                }).join('')
            }
        </div>`
    }

    content += `<div class="side-info-item players-titles">
        ${ side.players.map(pl => `<div>${ pl.title }</div>`).join('') }
    </div>`
