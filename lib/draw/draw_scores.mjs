export const draw_scores = (
    ...
    ctx.fillStyle = sides[0].id === selected_team_id ? options.highlighted_team_text_color : options.score_text_color
    scores_middle_Y = center_Y - offset
    sides?.[0]?.score?.forEach?.(draw_single_score)

    ctx.fillStyle = sides[1].id === selected_team_id ? options.highlighted_team_text_color : options.score_text_color
    scores_middle_Y = center_Y + offset
