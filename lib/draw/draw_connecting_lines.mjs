...
// draw regular lines
    if (parent_to_highlight === null) {
        ...
        if (options.connection_lines_type === 'diagonal' || is_match_visible_Y(farthest_parent, ctx.canvas.height)) {
            draw_line_to(farthest_parent)
            ctx.moveTo(...current_coords)
        }
        if (options.connection_lines_type === 'diagonal' || is_match_visible_Y(closest_parent, ctx.canvas.height)) {
