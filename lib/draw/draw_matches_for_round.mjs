...
export const draw_matches_for_round = (
    ...
    round_data.matches.forEach((match, match_index) => {
        const center_Y = Math.floor(match.center_Y)
        ...
        if (should_draw_seed(round_index, options)) {
            draw_seed(match.sides, current_X, center_Y, options, ctx)
            current_X += options.seed_width
        }

        draw_teams_titles(match.sides, current_X, center_Y, options, ctx)
...
// draw 'axis' of the match
        ...
        const team_id_to_highlight = match.sides.find(s => s.id === state.selected_team_id)?.id
