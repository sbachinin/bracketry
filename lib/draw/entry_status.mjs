...
export const should_draw_entry_status = (round_index, options) => {
    ...
    if (options.draw_entry_status === 'Never') return false
    if (options.draw_entry_status === 'At all rounds') return true
    if (options.draw_entry_status === 'At first round') return round_index === 0
}

export const draw_entry_status = ({ match, options, ctx }) => {
    const offset = get_team_texts_offset(options)

    if (options.draw_entry_status) {
