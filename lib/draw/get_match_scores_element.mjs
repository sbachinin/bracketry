const get_single_side_score_el = (side, score_index) => {
    ...
    return `<div>${main_score_el}${tie_break_el}</div>`
}

export const get_match_scores_element = (match) => {
    let pairs = [
        /* dummy el with proper height */
        '<div class="single-scores-pair dummy"><div>&#8203;</div><div>&#8203;</div></div>'
    ]
    ...
    if (Array.isArray(match.sides) && match.sides.length) {
        ...
        while (match.sides.find(side => side.score?.[score_index] !== undefined)) {
            ...
            const pair = `<div class="single-scores-pair">${upper_score_el}${lower_score_el}</div>`
            pairs.push(pair)
            score_index++
        }
    }
    ...
    return `
        <div class="match-scores">${pairs.join('')}</div>
    `
}