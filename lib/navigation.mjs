...
const get_visible_rounds_count = (shell, get_option) => {
    ...
    if (vrc_option !== undefined && vrc_option >= 1) {
        ...
        max_count = Math.floor(shell.content_area.clientWidth / shell.matches_positioner.firstChild.clientWidth)
    }
...
// remember scrollY ratio
    ...
    const area_width = shell.content_area.clientWidth
...
// reset rounds widths to measure 1st round's natural width
    ...
    const first_round_width = rounds[0].clientWidth
...
// make whole number of rounds if options.VRC is provided
        ...
        if (vrc_option !== undefined && vrc_option >= 1) {
            const would_be_width = Math.ceil(
                area_width / Math.min(rounds.length, vrc_option)
            ) + 'px'
