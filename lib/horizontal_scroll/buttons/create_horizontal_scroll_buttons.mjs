...
export const create_horizontal_scroll_buttons = (root_id, options) => {
    ...
    const update_visibility = debounce((all_data, options, scroll_X_anchor_round_index) => {
        const is_visible = options.horizontal_scroll_triggered_by === 'buttons'
        left_button.wrapper.classList[is_visible ? 'remove' : 'add']('hidden')
        right_button.wrapper.classList[is_visible ? 'remove' : 'add']('hidden')
        
        const left_is_disabled = scroll_X_anchor_round_index <= 0
        const right_is_disabled = scroll_X_anchor_round_index >= get_max_scroll_round_index(all_data, options)
