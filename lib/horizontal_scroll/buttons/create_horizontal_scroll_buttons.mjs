...
import { get_width_per_round, get_max_scroll_round_index } from '../../utils/sizes.mjs'
...
export const create_horizontal_scroll_buttons = (the_root_element, root_id, options) => {
    ...
    const update_visibility = debounce((all_data, options, anchor_round_index) => {
        const should_show = get_width_per_round(options) * all_data.rounds.length > the_root_element.clientWidth
        left_button.wrapper.classList[should_show ? 'remove' : 'add']('hidden')
        right_button.wrapper.classList[should_show ? 'remove' : 'add']('hidden')
        
        const left_is_disabled = anchor_round_index <= 0
        const right_is_disabled = anchor_round_index >= get_max_scroll_round_index(the_root_element, all_data, options)
