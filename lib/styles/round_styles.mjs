export const get_rounds_styles = (root_id, options) => `
    ...
    #${root_id} .match-info-pair .side-info-item {
        display: flex;
        align-items: center;
    }
    
    #${root_id} .match-info-pair:not(.players-titles) * {
        justify-content: center;
    }

    #${root_id} .match-info-pair.players-titles .side-info-item.winner {
        font-weight: ${ options.winner_is_bold ? '700' : '400' }
    }

    #${root_id} .side-info-item .height-maker {
        font-family: ${ options.player_title_font_family };
        font-size: ${ options.match_root_font_size }px;
    }

    #${root_id} .match-info-pair.nationalities img {
        width: 100%;
        max-height: ${options.match_root_font_size * 0.85}px;
    }
