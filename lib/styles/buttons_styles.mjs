...
export const get_buttons_styles = (root_id, options) => {
    ...
    const buttons_are_header = options.horizontal_scroll_buttons_position === 'Before round titles'
    ...
    return `
        ...
        #${root_id} .buttons-header {
            ...
            background: tomato;
            display: ${buttons_are_header ? 'block' : 'none'};
        }
        ...
        @media (max-width: ${SWAP_BUTTONS_POSITION_THRESHOLD}px) {
            #${root_id} .buttons-header {
                display: ${buttons_are_hidden ? 'none' : 'block'};
            }
            #${root_id} .scroll-button {
                display: none;
            }
