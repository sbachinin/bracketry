...
// set width and offset according to round index
    shell.matches_scroller.style.width = 'max-content' // let rounds attain natural width
    ...
    shell.matches_scroller.style.width = new_width
    shell.matches_scroller.style.marginLeft = new_marginleft
...
export const create_navigation = (shell, get_option, scrolla) => {
    ...
    const repaint_on_image_load = debounce((e) => {
        ...
        // TODO: chromium warns that e.path is deprecated and suggests replacing it with e.composedPath but there is no such prop there
        if (
            e.path
            && e.path[0]?.closest(`.playoffs-root .content-area`)
