...
export const handle_new_left = (initial_round_width, els, base_index) => {
    ...
    const visible_rounds_count = Math.max(
        1,
        Math.floor(els.content_horizontal_scroller.clientWidth / initial_round_width)
    )
...
export const create_navigation = (root_id) => {
    ...
    let base_index = 0
    const initial_round_width = els.matches_scrollable_area.firstChild.clientWidth

    handle_new_left(initial_round_width, els, base_index)
    ...
    return {
        ...
        repaint: () => handle_new_left(initial_round_width, els, base_index),
