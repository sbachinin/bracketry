...
export const install_mouse_events = (
    ...
    canvas_el.addEventListener('wheel', e => {
        ...
        // 1 scroll step === 1 match height, but expressed in percents of height deficit
        const delta = e.deltaY
            * options.min_height_per_match
            / longest_round_height_deficit

        const initial_scroll_Y = state.scrollY
        const new_scroll_Y = within_range(initial_scroll_Y + delta, 0, 100)
        const distance = new_scroll_Y - initial_scroll_Y

        animate_with_easing(
            easing_value => {
                handle_new_scrollY(initial_scroll_Y + distance * easing_value)
            }
        )
    })
}
