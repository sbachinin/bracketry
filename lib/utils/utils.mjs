...
export const update_style = (root_id, style_id, styles) => {
    const current_styles_node = document.head.querySelector(`#${root_id}-${style_id}`)
    if (current_styles_node) document.head.removeChild(current_styles_node)
    
    document.head.insertAdjacentHTML(
        'beforeend',
        `<style id='${root_id}-${style_id}'>${styles}</style>`
    )
}
