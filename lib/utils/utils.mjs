...
export const find_mouse_position_within_element = e => {
...
export const set_element_style = (el, style) => {
    Object.entries(style).forEach(([name, val]) => {
        el.style[name] = val
    })
}
...
export const user_agent_is_mobile = () => /Mobi|Android/i.test(navigator.userAgent)

// BEFORE CALLING THIS, 'MOBILE' CLASS IS REMOVED FROM THE_ROOT_ELEMENT, OTHERWISE WIDTH MEASUREMENT MAKES LITTLE SENSE
export const should_be_mobile_layout = (
    { the_root_element, content_horizontal_scroller },
    get_option
) => {
    if (get_option('useMobileLayout')) {
// mobile layout is forced
        return true
    }
    
    const round_to_measure = (
        the_root_element.querySelector('.round-wrapper.anchor-round')
        || the_root_element.querySelector('.round-wrapper')
    )
    if (round_to_measure === null) {
        return false
    }
// mobile layout if visible area is too narrow
    return (round_to_measure.clientWidth + get_option('matchHorMargin')*2) >= content_horizontal_scroller.clientWidth
}