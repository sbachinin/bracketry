...
export const draw_rounds = (
    ...
    all_rounds.forEach((_, round_index) => {
        const { left_X } = basic_drawing_attrs.rounds_drawing_attrs[round_index]
        if ( // this round is invisible under current scroll_X
            (left_X + basic_drawing_attrs.round_width) < -state.scroll_X
            || left_X > (-state.scroll_X + canvas_el.width)
        ) return
    ...
    all_rounds.forEach((_, round_index) => {
        const { title, left_X } = basic_drawing_attrs.rounds_drawing_attrs[round_index]
        ctx.fillText(
            title,
            (
