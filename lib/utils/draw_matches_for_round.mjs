...
export const draw_matches_for_round = (round_index, all_rounds, ctx, options) => {
    ...
    all_rounds[round_index].matches_to_draw.forEach((match_data, match_index) => {
        ...
        // ctx.fillRect(
        //     positionX + options.margin_between_rounds,
        //     match_body_Y,
        //     match_body_width,
        //     match_body_height)
        ...
        ctx.fillText(
            first_team_title,
            positionX + options.margin_between_rounds + constants.MATCH_HOR_PADDING,
        ...
        ctx.moveTo(
            positionX + options.round_width - options.margin_between_rounds,
            centerY
        );
        ...
        if (round_index > 0) {
            ...
            if (options.connection_lines_type.includes('bended')) {
                connection_point = [
                    connection_point[0] - options.margin_between_rounds - options.connection_lines_width,
