...
export const get_match_body_height = (options) => {
    return VERT_GAP_BTW_OPPONENTS + options.team_title_font_size * 2.5
}
...
export const measure_widths = (rounds, options, ctx) => {
    ...
    const round_width = MATCH_PADDING_LEFT
        + (options.entry_status_visibility === visibility_options.ALL_ROUNDS && !is_match_reduced ? options.entry_status_width : 0)
        + (options.nationalities_visibility === visibility_options.ALL_ROUNDS && !is_match_reduced ? options.nationality_width : 0)
        + TEAM_TITLE_LEFT_MARGIN
        + team_title_width
    ...
    if (options.entry_status_visibility === visibility_options.FIRST_ROUND && !is_match_reduced) {
        first_round_width += options.entry_status_width
    }
    if (options.nationalities_visibility === visibility_options.FIRST_ROUND && !is_match_reduced) {
        first_round_width += options.nationality_width
    }
...
export const get_team_texts_offset = options => {
    return Math.floor(VERT_GAP_BTW_OPPONENTS/2 + options.team_title_font_size/2)
}
...
export const measure_main_canvas = (wrapper, first_round_length, options) => {
    ...
    const canvas_vert_margin = (
        ...
        ? options.vertical_scroll_buttons_clickable_height
        : 0
