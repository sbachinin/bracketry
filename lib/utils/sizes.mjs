...
export const measure_widths = (rounds, options, ctx) => {
    ...
    const round_width = options.match_padding_left
        + (options.draw_entry_status === 'At all rounds' && !is_match_reduced ? options.entry_status_width : 0)
        + (options.draw_nationalities === 'At all rounds' && !is_match_reduced ? options.nationality_width : 0)
    ...
    if (options.draw_entry_status === 'At first round' && !is_match_reduced) {
        first_round_width += options.entry_status_width
    }
    if (options.draw_nationalities === 'At first round' && !is_match_reduced) {
