...
export const get_height_deficit_for_round = (
    ...
    return (matches_count * options.min_height_per_match + options.matches_padding_bottom) - canvas_height
}
...
export const measure_round_heights = (
    ...
    const height_per_match = Math.max(
        ...
        (canvas_height - options.matches_padding_bottom) / matches_count
    )
...
export const get_match_center_scroll_Y = (round, scroll_Y, match_index) =>{ 
    return round.height_per_match * (match_index + 0.5) - get_round_scroll_Y_px(round, scroll_Y)
}