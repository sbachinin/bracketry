...
export const get_match_body_height = (options) => {
    return options.vert_gap_between_opponents + options.team_title_font_size * 1.5
}
...
export const measure_widths = (rounds, options, ctx) => {
    ...
    const scores_width = options.scores_left_margin
            + (options.score_font_size + options.score_hor_margin*2) * longest_score_length

    const round_width = options.match_padding_left
        + (options.draw_seed === 'At all rounds' ? options.seed_width : 0)
        + team_title_width
        + (options.show_scores_only_on_hover ? 0 : scores_width)
