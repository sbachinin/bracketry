...
export const get_max_scroll_round_index = (the_root_element, all_data, options) => {
    const fully_visible_rounds_count = Math.floor(
        (the_root_element.clientWidth - options.horizontal_scroll_buttons_clickable_width * 2)
        / get_width_per_round(options)
    )
    return all_data.rounds.length - fully_visible_rounds_count
}

export const get_width_per_round = (options) => (options.match_width + options.match_hor_margin * 2)
...
export const get_round_title_left_X = (round_left_X, options) => {
    switch (options.round_title_text_align) {
        ...
        case 'center': return round_left_X + get_width_per_round(options) / 2
        case 'right': return round_left_X + get_width_per_round(options) - options.round_title_hor_margin - options.match_hor_margin
    }
}
