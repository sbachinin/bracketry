...
export const get_height_per_match = (round, scroll_X, options) => {
    let height_per_match = options.min_vert_margin_between_matches + get_match_body_height(options)
    
    const { anchor_height_per_match, anchor_round_height_deficit } = analyze_anchor_round_height(all_data, state, options)
    if (scroll_X <= round.left_X) {
        height_per_match = anchor_height_per_match
    }

    return height_per_match
}
...
export const get_match_center_Y = (all_data, round_index, match_index, state, options) => {
    ...
    const anchor_scroll_point = all_data.rounds[round_index].left_X - options.distance_between_rounds / 2
    
    // how far is this round's left_X from scroll_X, expressed in number of rounds
    const round_distance_from_anchor = (anchor_scroll_point - state.scroll_X) / (all_data.round_width + options.distance_between_rounds)
