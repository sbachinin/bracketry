...
export const measure_widths = (rounds, options, ctx) => {
    ...
    const round_width = options.match_padding_left
        + (options.should_draw_entry_status === 'At all rounds' && !is_match_reduced ? options.entry_status_width : 0)
        + (options.should_draw_nationalities === 'At all rounds' && !is_match_reduced ? options.nationality_width : 0)
        + options.team_title_left_margin
        + team_title_width
        + (is_match_reduced ? 0 : scores_width + options.scores_left_margin)
        + options.match_padding_right

    let first_round_width = round_width
    if (options.should_draw_entry_status === 'At first round' && !is_match_reduced) {
        first_round_width += options.entry_status_width
    }
    if (options.should_draw_nationalities === 'At first round' && !is_match_reduced) {
