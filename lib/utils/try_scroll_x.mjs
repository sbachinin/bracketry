...
export const tryScrollX = (
    all_data,
    state,
    handle_new_scrollX,
    canvas_el,
    round_width_with_margin,
    e
) => {
    const content_width = all_data.rounds.length * round_width_with_margin

    if (content_width - canvas_el.width <= 0) return

    scroll_force = get_scroll_force(canvas_el, e)

    make_scroll_step = () => {
        const new_scrollX = get_scrollX_with_constraints(state, content_width, canvas_el);
