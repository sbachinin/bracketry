...
export const draw_all = (all_rounds_data, state, canvas_el, options) => {
                ...
                matches: round_data.matches
                    .map(match_data => {
                        widest_team_title_width = Math.max(
                            ...
                            ctx.measureText(match_data.sides[0].title).width,
                            ctx.measureText(match_data.sides[1].title).width
                        )
                        return {
                            ...match_data,
