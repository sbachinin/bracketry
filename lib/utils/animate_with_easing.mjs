...
let last_animations_of_type = {} // { [type]: [id] }

// this fn calls handle_new_value every frame for a given duration with numbers from 0 to (almost?) 1
// this fn stops ongoing animation OF SPECIFIC TYPE before starting a new one!
export const animate_with_easing = ({ type, handle_new_value, duration }) => {
    let initial_timestamp = null
    
    const this_animation_id = create_unique_id()
    last_animations_of_type[type] = this_animation_id
    ...
    const move = raf_timestamp => {
        if (last_animations_of_type[type] !== this_animation_id) return
