...
const create_scroll_buttons = (the_root_element) => {
    ...
    return {
        ...
        update_innerHTML(options) {
            left.innerHTML = options.leftNavigationButtonHTML
            top_left.innerHTML = options.leftNavigationButtonHTML
            right.innerHTML = options.rightNavigationButtonHTML
            top_right.innerHTML = options.rightNavigationButtonHTML
        },
        update_hidden(content_is_wider, options) {
            const method = content_is_wider || options.useMobileLayout ? 'remove' : 'add'
