...
const create_scroll_buttons = (the_root_element) => {
    ...
    return {
        ...
        update_innerHTML(options) {
            left.innerHTML = options.left_navigation_button_html
            top_left.innerHTML = options.left_navigation_button_html
            right.innerHTML = options.right_navigation_button_html
            top_right.innerHTML = options.right_navigation_button_html
        },
        update_hidden(content_is_wider, options) {
            const method = content_is_wider || options.use_mobile_layout ? 'remove' : 'add'
