...
const create_navigation_buttons = (stable_elements) => {
    ...
    return {
        ...
        update_active(stable_elements, base_round_index, rounds_count) {
            let left_is_active = base_round_index > 0
            let right_is_active = base_round_index < (rounds_count - 1)
            
            const is_mobile = stable_elements.the_root_element.classList.contains('mobile')
            if (!is_mobile) {
                left_is_active = parseInt(stable_elements.content_area.style.left) < 0
                right_is_active = has_reached_right_edge(stable_elements)
            }
