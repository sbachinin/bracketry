...
export const restrict_native_scroll = (el, make_scroll_jump) => {
    ...
    if (el === window) {
        // touchmove must be disabled only on fullscreen
        // because disabling touchmove on 'static' matches-scroller leads to unwanted:
        // window scroll gets blocked when touching over playoffs; and it only degrades UI.
        // (in such case "mixed" scroll mode will only allow buttons but will spoil window scroll without reason)
