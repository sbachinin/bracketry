export const CURSOR_EFFECT = 'cursor'
export const SCROLL_EFFECT_NAME = 'scroll'
export const SCROLL_X_EFFECT = 'scroll_X'
export const ANCHOR_SCROLL_ROUND_EFFECT = 'anchor_scroll_round'
export const REDRAW_EFFECT = 'redraw'
export const EXPAND_MATCH_EFFECT = 'expand_match'

const run_effects = (state, old_state, force_redraw, effects) => {
    if (state.cursor !== old_state.cursor) effects[CURSOR_EFFECT]?.()
    
    if (state.scroll_X !== old_state.scroll_X) effects[SCROLL_X_EFFECT]?.()

    if (state.scroll_X_anchor_round_index !== old_state.scroll_X_anchor_round_index) {
        effects[ANCHOR_SCROLL_ROUND_EFFECT]?.()
    }

    if (state.scroll_Y !== old_state.scroll_Y
        || state.scroll_X !== old_state.scroll_X
    ) {
        effects[SCROLL_EFFECT_NAME]?.()
    }
