...
export const create_options = (orig_user_options) => {
    const all_options_meta = get_flattened_options()
...
// try fix options of wrong type
        if (option_meta.type === 'pixels' && typeof option_value !== 'number') {
...
// ensure min numbers
        if (option_meta.type === 'pixels' && typeof option_meta.min_value === 'number') {
    ...
    if (true /* applyDevicePixelRatio */) {
        Object.entries(actual_options).forEach(([option_name, option_value]) => {
            const option_meta = all_options_meta[option_name]
            if (option_meta && option_meta.type === 'pixels') {
                actual_options[option_name] = option_value * devicePixelRatio
            }
        })
    }
