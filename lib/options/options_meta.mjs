import * as explanations from './explanations.mjs'
...
/* 
    auto_content_size: {
        title: 'Auto canvas size (as high and as wide as the entire content)',
        type: 'boolean',
        default_value: false,
        explanation: explanations.auto_content_size
    }, */
...
export const MATCH_OPTIONS = {
    ...
    onMatchClick: {
        title: 'What happens when you click on a match (function)',
        type: 'function_or_null',
        default_value: null,
        explanation: `
Here you can provide a function that will be called with a match object.

For example, match element can become a link to match's dedicated page:

onMatchClick: (match) => window.location = \`matches/\${match.id}\`

It will actually be called with 3 arguments:

onMatchClick: (match, round, round_index) => ...

If this handler is provided, it will stop team's tournament path from being highlighted on click`
    },

    onTeamTitleClick: {
        title: 'What happens when you click on a team title (function)',
        type: 'function_or_null',
        default_value: null,
        explanation: `
Here you can provide a function that will be called with a team object.

For example, team title can become a link to team's dedicated page:

onTeamTitleClick: (match) => window.location = \`teams/\${team.id}\`

If this handler is provided, it will stop team's tournament path from being highlighted on click`
    },

    min_vertical_distance_btw_matches: {
        title: 'Minimal horizontal distance between matches, in pixels',
        type: 'number',
        default_value: 25,
    },

    vertical_distance_between_teams: {
        title: `Vertical distance between two sides of a match (even number of pixels)`,
        type: 'number',
        default_value: 3
    },

    show_winner_mark: {
        title: `Show winner mark if team.result === "winner"`,
        type: 'boolean',
        default_value: false
    },

    winner_mark: {
        title: 'Winner mark (HTML string with <svg> or whatever)',
        type: 'multiline_string',
        default_value: `
            <svg class="default-winner-mark" style="fill: #00a5de" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M9 21.035l-9-8.638 2.791-2.87 6.156 5.874 12.21-12.436 2.843 2.817z"/></svg>
        `,
        disable_if: options => !options.show_winner_mark
    },
...
export const FONTS_OPTIONS = {
    ...
    winner_is_bold: {
        title: `Apply { font-weight: bold } if team.result === "winner"`,
        type: 'boolean',
        default_value: true,
    }
}
