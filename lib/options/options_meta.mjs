...
import { get_root_folder } from '../utils/utils.mjs'
...
const GENERAL_OPTIONS = {
    useMobileLayout: {
        ...
        type: 'boolean',
        default_value: false,
        expanation: `If true, only one round is shown even if there is enough space for more rounds`
    },
 ...
 */
    ...
    applyDevicePixelRatio: {
        title: 'Apply device pixel ratio',
        type: 'boolean',
        default_value: true,
        explanation: `
True means that easy-playoffs will multiply all pixel options values (font-sizes, margins etc) by Window.devicePixelRatio.
<br />
This will (hopefully) make actual physical sizes more or less consistent between desktop and mobile devices.
<br />
Otherwise everything will appear much smaller on devices with high pixel density.
<br />
Setting applyDevicePixelRatio to true won't make the physical sizes precisely equal but at least will reduce the impact of varying pixel density.
<br />
<a href="/${get_root_folder()}pages/mobile-layout.html#device-pixel-ratio" class="more-link">More</a>
`
    }
}
...
Note that this max-width will be only applied when 2+ rounds are visible.
On "mobile" (1-round) layout this will have no effect; there matches will remain fluid: i.e., they can grow a bit wider than matchMaxWidth if there's plenty of free width`
    },
