import { get_flattened_options, get_all_options_names } from './get_default_options.mjs'
...
// ensure min numbers
    Object.entries(user_options).forEach(([option_name, option_value]) => {
        const option_meta = flattened_options[option_name]
        if (option_meta.type === 'number' && typeof option_meta.min_value === 'number') {
            actual_options[option_name] = Math.max(option_value, option_meta.min_value)
        }
    })
