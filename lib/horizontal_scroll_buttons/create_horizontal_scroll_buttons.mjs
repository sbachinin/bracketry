...
export const create_horizontal_scroll_buttons = (
    ...
    const update_round_index = change => {
        ...
        const new_anchor_index = get_new_anchor_index(
            change, all_data.rounds, width_deficit, options, store)
        store.update_state({ scroll_X_anchor_round_index: new_anchor_index })

        const destination_scroll_X = new_anchor_index > 0
            ? Math.min(
                width_deficit,
                get_round_left_X(
                    options,
