...
export const create_horizontal_scroll_buttons = (
    ...
    const handle_new_round_index = new_index => {
        ...
        const width_deficit = rounds_count * options.round_width - root_bracket_container.clientWidth
        const initial_scrollX = state.scrollX
        const destination_scrollX = -Math.min(width_deficit, new_index * options.round_width)
    ...
    const get_invisible_rounds_count = () => {
        const fully_visible_rounds_count = Math.floor(
            root_bracket_container.clientWidth / options.round_width
        )
