...
// create canvas for round titles
    const title_canvas_el = document.createElement('canvas')
    title_canvas_el.style.border = 'none'
    root_brackets_el.append(title_canvas_el)
...
// set updater
    const update_root_elements = (options, longest_round_match_count) => {
        ...
        canvas_el.height = root_brackets_el.clientHeight - get_round_title_height(options)
        if (options.auto_canvas_height) {
            canvas_el.height = get_total_rounds_height(options, longest_round_match_count)
        }
        ...
        title_canvas_el.width = root_brackets_el.clientWidth
        title_canvas_el.height = get_round_title_height(options)
        title_canvas_el.style.backgroundColor = options.background_color
    }
