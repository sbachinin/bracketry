...
export const create_html_shell = (user_wrapper_el, root_id) => {
    [...user_wrapper_el.querySelectorAll('.easy-playoffs-root')].forEach(epr => epr.remove())

    update_styles(
        'root',
        'permanent-styles',
        (
            get_general_permanent_styles()
            + get_buttons_permanent_styles()
            + get_rounds_permanent_styles()
        ).replace(/&&&/g, '.easy-playoffs-root')
    )

    const the_root_element = create_element_from_Html(`
        <div class="easy-playoffs-root" id=${root_id}>
