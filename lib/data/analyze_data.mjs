...
export const ananlyze_data = (all_rounds_data, options) => {
    let have_entry_status = false
    let have_nationality = false
    ...
    all_rounds_data.forEach(round => {
        ...
        round.matches?.forEach(match => {
            ...
            match.sides.forEach?.(side => {
                ...
                if (typeof side.entry_status !== 'undefined') {
                    if (typeof side.entry_status === 'string') {
                        have_entry_status = true
                    } else {
                        halt('If side.entry_status is provided, it must be a string: ', side)
                    }
                }

                if (typeof side.nationality_code !== 'undefined') {
                    if (typeof side.nationality_code === 'string') {
                        have_nationality = true
                    } else {
                ...
                if (typeof side.title !== 'string') {
                    halt('Side.title must be a string: ', side)
                }
