...
export const ananlyze_data = (all_data) => {
    ...
    Object.values(all_data.contestants).forEach(contestant_meta => {
        if (typeof contestant_meta.title !== 'string') {
            halt('Team must have a title, and it must be a string: ', contestant_meta)
        }

        if (typeof contestant_meta.entry_status !== 'undefined' && typeof contestant_meta.entry_status !== 'string') {
            halt('If entry_status is provided for a team, it must be a string: ', contestant_meta)
        }

        if (typeof contestant_meta.nationality_code !== 'undefined' && typeof contestant_meta.nationality_code !== 'string') {
            // may also check for the length of this code...
            halt('If nationality_code is provided for a team, it must be a string: ', contestant_meta)
        }

        if (typeof contestant_meta.flag_url !== 'undefined' && typeof contestant_meta.flag_url !== 'string') {
            halt('If flag_url is provided for a team, it must be a string: ', contestant_meta)
        }
    })
}
