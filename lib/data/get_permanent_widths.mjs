...
export const get_permanent_widths = (rounds, options, ctx) => {
    ...
    const round_width = MATCH_PADDING_LEFT
        + Math.floor(widest_entry_status_width)
        + options.match_font_size*2 // width of nationality/flag
        + TEAM_TITLE_LEFT_MARGIN
        + Math.floor(widest_team_title_width)
        + SCORES_LEFT_MARGIN
        + scores_width
        + MATCH_PADDING_RIGHT
    
    return {
        team_title_width: Math.floor(widest_team_title_width),
        scores_width,
        entry_status_width: Math.floor(widest_entry_status_width),
