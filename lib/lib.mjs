...
const apply_new_options = (
    ...
    stable_elements.navigation_buttons.update_innerHTML(options_dealer)
    update_all_styles(all_data.root_id, options_dealer)
}
...
export const createPlayoffs = (user_data, user_wrapper_el, user_options) => {
    const options_dealer = create_options_dealer(user_options)
    ...
    store.set_effects(get_effects(store, all_data, options_dealer, stable_elements))

    store.update_state({ anchor_round_index: get_initial_anchor_index(all_data, stable_elements, options_dealer) })
    ...
    observe_resize_later(
        user_wrapper_el,
        () => handle_round_index(all_data, options_dealer, store.state, stable_elements)
    )
...
// expose stuff
    return {
        ...
        moveToNRound: (n) => {
            let new_index = n
            if (!options_dealer.get_final_value('useMobileLayout')) {
            ...
            try_update_anchor_round_index(new_index, all_data, store, stable_elements, options_dealer)
        },
        ...
        getNavigationState: () => {
            ...
            return {
                ...
                reachedRightEdge: (
                    get_max_anchor_round_index(stable_elements, all_data, options_dealer)
