...
const get_initial_anchor_index = (all_data, root_elements, options) => {
...
export const createBrackets = (initial_data, user_wrapper_el, user_options) => {
    ...
    const full_update = async (original_user_data = initial_data, user_options = {}) => {
        ...
        full_redraw_matches(all_data, actual_options, store, root_elements)
        store.update_state({
            anchor_round_index: get_initial_anchor_index(all_data, root_elements, actual_options)
        })
    }
    ...
    return {
        full_update,
        scroll_left: () => try_update_scroll_round_index(-1, all_data, store, root_elements, actual_options),
        scroll_right: () => try_update_scroll_round_index(1, all_data, store, root_elements, actual_options)
    }
}
