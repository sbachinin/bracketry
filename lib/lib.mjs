...
const try_assign_new_data = (new_data, old_data) => {
    ...
    Object.assign(
        old_data,
        deep_clone_object(new_data),
        get_permanent_drawing_props(new_data)
    )
...
export const createPlayoffs = (user_data, user_wrapper_el, user_options) => {
    ...
    // TODO rename to html_shell
    const stable_elements = create_html_shell(user_wrapper_el, root_id)
    apply_new_options(user_options, options_dealer, root_id, stable_elements)
    ...
    render_content(all_data, stable_elements, options_dealer.get_final_value)
...
// expose stuff
    return {
        ...
        applyNewOptions: (new_options) => {
            apply_new_options(new_options, options_dealer, root_id, stable_elements)
        ...
        replaceData: (new_data) => {
            ...
            stable_elements.matches_vertical_scroller.scrollTop = 0
        },
        ...
        applyMatchesUpdates: (fresh_matches) => {
            ...
            fresh_matches.forEach(m => {
                ...
                const m_i = all_data.matches.findIndex((old_match) => {
                    return old_match.round_index === m.round_index && old_match.order === m.order
                })
                ...
                const round_el = document.querySelectorAll(`#${root_id} .round-wrapper`)[m.round_index]
        ...
        // TODO: highlightContestantHistory
    }
}
