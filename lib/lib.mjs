...
export const createBrackets = (initial_data, root_container, user_options) => {
    ...
    const update_all = (user_data = initial_data, user_options = actual_options) => {
        ...
        draw_round_titles(all_data, store.state, actual_options, round_titles_canvas_el)
    ...
    store.onchange((state, old_state, force_redraw) => {
        ...
        if (includes_some(changed_state_keys, ['cursor'])) {
            main_canvas_el.style.cursor = state.cursor
        }
        
        if (includes_some(changed_state_keys, ['scroll_X'])) {
            buttons.update_visibility()
            setTimeout(() => draw_round_titles(all_data, store.state, actual_options, round_titles_canvas_el))
        }

        if (includes_some(changed_state_keys, [
            'scroll_Y', 'scroll_X', 'expanded_match_id', 'expanded_match_opacity', 'highlighted_team_id'
        ]) || force_redraw) {
