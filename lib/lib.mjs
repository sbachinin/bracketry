...
import { create_unique_id, deep_clone_object, create_element_from_Html, is_object } from './utils.mjs'
    ...
    return {
        ...
        applyMatchesUpdates: (matches_data) => {
            ...
            matches_data.forEach(m => {
                ...
                const round_el = document.querySelector(`#${all_data.root_id} .round-wrapper[round-id="${m.round_id}"]`)
                const match_el = round_el.querySelectorAll('.match-wrapper')[m.order]
                if (!match_el) return
                match_el.replaceWith(
                    get_match_element(m.round_id, m.order, all_data, options_dealer.get_final_value)
                )
            })
        },
