...
export const createBrackets = (initial_data, root_container, user_options) => {
    ...
    const onresize = (_canvas_el) => {
        set_round_heights(all_data, actual_options, _canvas_el.height)
        buttons.update_visibility()
        draw_rounds(all_data, store.state, _canvas_el, actual_options)
    }
    const {
        root_brackets_el, canvas_el, update_root_elements
    } = create_root_elements(root_id, actual_options, onresize)
