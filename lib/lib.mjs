...
const try_get_flag_imgs = flag_urls => {
    const promises = flag_urls.map(url => {
        return new Promise(async resolve => {
            const img = await get_image_from_URL(url)
            resolve({ [url]: img })
        })
    })
    ...
    return Promise.all(promises).then(arr => {
        const obj = {}
        arr.forEach(url_to_img => Object.assign(obj, url_to_img))
        return obj
    })
}
...
export const createBrackets = (initial_data, root_container, user_options) => {
    ...
    const update_all = async (user_data = initial_data, user_options = {}) => {
        ...
        Object.assign(all_data, { flags_urls_to_images })
