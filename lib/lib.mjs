...
import { add_permanent_drawing_data } from './data/add_permanent_drawing_data.mjs'
import { set_matches_coords, try_update_matches_coords } from './utils/set_matches_coords.mjs'
...
export const createBrackets = (
    ...
    const all_rounds_data = add_permanent_drawing_data(
        JSON.parse(JSON.stringify(initial_data)),
        actual_options,
        canvas_el
    )

    // make heavy calculations here to prevent doing it on drawing
    ...
    set_matches_coords(all_rounds_data, 0, basic_drawing_attrs, actual_options)

    const update_state = (update = {}) => {
        try_update_matches_coords(
            all_rounds_data,
            state,
            update,
            basic_drawing_attrs,
