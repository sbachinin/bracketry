...
import { try_navigate } from './navigation/try_navigate.mjs'
...
// expose stuff
    return {
        moveToPreviousRound: () => try_navigate(
            store.state.base_round_index-1,
            all_data,
            store,
            stable_elements),

        moveToNextRound: () => try_navigate(
            store.state.base_round_index+1,
            all_data,
            store,
            stable_elements),
        ...
        setCentralRoundIndex: (n) => {
            ...
            try_navigate(new_index, all_data, store, stable_elements)
        },

        setBaseRoundIndex: (n) => {
            try_navigate(n, all_data, store, stable_elements)
        },
