...
export const createBrackets = (initial_data, user_wrapper_el, user_options) => {
    ...
    const run_full_update = async (original_user_data = initial_data, user_options = {}) => {
        ...
        store.update_state({
            first_round_width: measure_first_round(all_data, root_elements, actual_options)
        })
    ...
    store.update_state({
        anchor_round_index: get_initial_anchor_index(all_data, root_elements, store.state)
    })
