...
// expose stuff
    return {
        moveToPreviousRound: () => try_update_scroll_round_index(store.state.anchor_round_index-1, all_data, store, stable_elements, actual_options),

        moveToNextRound: () => try_update_scroll_round_index(store.state.anchor_round_index+1, all_data, store, stable_elements, actual_options),

        moveToNRound: (n) => {
            let new_index = n
            if (!actual_options.use_mobile_layout) {
            ...
            try_update_scroll_round_index(new_index, all_data, store, stable_elements, actual_options)
        },
        ...
        getNavigationState: () => {
            ...
            return {
                ...
                reachedRightEdge: (
                    get_max_scroll_round_index(stable_elements, all_data, actual_options)
