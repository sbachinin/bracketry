...
export const createPlayoffs = (user_data, user_wrapper_el, user_options) => {
    ...
    const merge_succeeded = try_assign_new_data(all_data, user_data)
...
// expose stuff
    return {
        ...
        replaceData: (new_data) => {
            if (!alive) return
            const merge_succeeded = try_assign_new_data(all_data, new_data)
            if (!merge_succeeded) {
        ...
        applyMatchesUpdates: (updates) => {
            ...
            updates.forEach(u => {
                ...
                if (m_i === -1) {
                    ...
                    Object.assign(all_data.matches[m_i], u)
                }
                ...
                match_el.replaceWith(
                    get_match_element(u.roundIndex, u.order, all_data, options_dealer.get_final_value)
                )
            })
            navigation.repaint()
        },
        ...
        getAllData: () => alive && deep_clone_object(all_data),
