...
import { get_round_titles_html } from './draw/get_round_titles_html.mjs'
...
const try_assign_new_data = (new_data, old_data) => {
    ...
    Object.keys(old_data).forEach(key => (key !== 'root_id') && delete old_data[key])
...
export const createPlayoffs = (user_data, user_wrapper_el, user_options) => {
    ...
    if (
        !merge_succeeded
        || !is_valid_wrapper_el(user_wrapper_el)
...
// expose stuff
    return {
        ...
        replaceData: (new_data) => {
            ...
            stable_elements.round_titles_wrapper.innerHTML = get_round_titles_html(all_data.rounds)
