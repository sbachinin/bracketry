...
import { create_unique_id, get_image_from_URL } from './utils/utils.mjs'
...
const mock_url = 'https://purecatamphetamine.github.io/country-flag-icons/3x2/US.svg'

const try_get_flag_imgs = () => {
    return new Promise(async resolve => {
        const img = await get_image_from_URL(mock_url)
        resolve({ [mock_url]: img })
    })
}
...
export const createBrackets = (initial_data, root_container, user_options) => {
    ...
    const update_all = async (user_data = initial_data, user_options = {}) => {
        ...
        const flags_images = await try_get_flag_imgs()
        Object.assign(all_data, { flags_images })
