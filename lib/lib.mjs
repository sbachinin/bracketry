...
const apply_new_options = (
    ...
    store.state.first_round_width = measure_first_round(all_data, stable_elements, actual_options)
}
...
export const createBrackets = (user_data, user_wrapper_el, user_options) => {
    ...
    apply_new_options(user_options, actual_options, all_data, store, stable_elements)
...
// expose stuff
    return {
        ...
        getScrollState: () => ({
            ...
            reachedRightEdge: store.state.anchor_round_index >= get_max_scroll_round_index(stable_elements, all_data, store.state),
            contentIsWider: store.state.first_round_width * all_data.rounds.length > stable_elements.content_horizontal_scroller.clientWidth
        }),
