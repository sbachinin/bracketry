...
export const createBrackets = (initial_data, root_container, user_options) => {
    ...
    const root_brackets_el = create_root_el(root_id)
    root_container.append(root_brackets_el)
    const { canvas_el, update_canvas } = create_canvas(root_brackets_el)
    ...
    const update = (user_data, user_options) => {
        const initial_data = JSON.parse(JSON.stringify(user_data))
        ...
        update_canvas(actual_options, initial_data[0].matches.length)
        
        Object.assign(all_data, get_permanent_drawing_props(initial_data, actual_options, canvas_el))
        set_round_heights(all_data, actual_options, canvas_el.height)
        set_matches_coords(all_data, 0, actual_options)

        if (actual_options.horizontal_scroll_triggered_by === 'buttons'
            && buttons === null) {
            buttons = create_horizontal_scroll_buttons(
                all_data,
                root_brackets_el,
                actual_options,
                store,
    ...
    update(initial_data, user_options)
