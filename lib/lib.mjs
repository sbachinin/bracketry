...
import { draw_all } from './utils/draw/draw_all.mjs'
...
const try_adjust_scroll_X_on_resize = (
    ...
    return Math.max(0, scroll_X - right_gap_width)
}
...
export const createBrackets = (
    ...
    let basic_drawing_attrs = extract_basic_drawing_attrs(
        all_rounds_data,
        actual_options,
        canvas_el
    )
    ...
    new ResizeObserver(
        debounce(resize_entry => {
            ...
            basic_drawing_attrs = extract_basic_drawing_attrs(
                all_rounds_data,
                actual_options,
                canvas_el
            )
