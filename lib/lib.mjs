...
const apply_new_data = (new_data, actual_data) => {
    const user_data = deep_clone_object(new_data)
    try {
        ananlyze_data(user_data)
    } catch (e) {
        console.error(e)
        // HAVE TO STOP EXECUTION AT ALL
    }
...
export const createBrackets = (user_data, user_wrapper_el, user_options) => {
    ...
    const root_elements = create_root_elements(user_wrapper_el, all_data.root_id)
    if (!root_elements.lines_canvas.getContext) return

    apply_new_data(user_data, all_data)
    ...
    return {
        ...
        applyNewData: (new_data) => {
            apply_new_data(new_data, all_data)
            redraw()
        }
    }
}
