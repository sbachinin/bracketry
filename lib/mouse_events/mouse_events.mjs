...
export const install_mouse_events = (
    ...
    root_elements.wrapper.addEventListener(
        ...
        e => {
            ...
            if (e.target === root_elements.main_canvas_el) {
                try_update_hovered_stuff(e, all_data, store, options)
                try_set_tooltip(e, all_data, store.state, options, store.update_state)
            }
        }
    )
    ...
    root_elements.main_canvas_el.addEventListener("mousedown", e => maybe_start_dragging(e, all_data, store, options));
