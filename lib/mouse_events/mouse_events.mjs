...
export const install_mouse_events = (
    ...
    matches_scrollable_area.addEventListener(
        ...
        e => {
            ...
            if (e.target.classList.contains('match-body') && options.onMatchClick !== null) {
                const { match, round } = find_match_data_by_id(all_data, e.target.closest('.match-wrapper').getAttribute('match-id'))
                options.onMatchClick(match, round)
            }
