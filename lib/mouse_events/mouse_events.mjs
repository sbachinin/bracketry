...
import { enable_window_scroll, disable_window_scroll } from './fullscreen_scroll.mjs'
...
export const install_mouse_events = (
    ...
    const exit_fullscreen = () => {
        ...
        setTimeout(() => {
            the_root_element.remove()
        }, 300)
        // TODO will it uninstall stuff?
    }

    the_root_element.addEventListener('click', handle_root_click)
    the_root_element.querySelector('.exit-fullscreen-button').addEventListener('click', exit_fullscreen)
    ...
    get_option('fullscreen') === true && disable_window_scroll()


    const uninstall = () => {
        the_root_element?.removeEventListener('click', handle_root_click)
        the_root_element.querySelector('.exit-fullscreen-button').removeEventListener('click', exit_fullscreen)
