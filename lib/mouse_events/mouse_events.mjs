...
export const install_mouse_events = (
    ...
    matches_scrollable_area.addEventListener(
        ...
        // on match click
            if (options.get_final_value('onMatchClick') !== null) {
                if (e.target.classList.contains('match-body')) {
                    const match_id = e.target.closest('.match-wrapper')?.getAttribute('match-id')
                    options.get_final_value('onMatchClick')(all_data.matches.find(m => m.match_id === match_id))
                }
                return
            }

        // on side click
            if (options.get_final_value('onMatchSideClick') !== null) {
                if (e.target.classList.contains('side-wrapper')) {
                    ...
                    options.get_final_value('onMatchSideClick')(contestant, contestant_id, match)
                }
                return
            }
    ...
    const navigate_left = e => {
        ...
        && try_update_anchor_round_index(store.state.anchor_round_index-1, all_data, store, stable_elements, options)
    }
    ...
    matches_vertical_scroller.addEventListener('scroll', (e) => {
        if (!options.get_final_value('showScrollbar')) return
