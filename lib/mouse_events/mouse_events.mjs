...
const switch_expanded_match = (e, all_data, state, options, update_state) => {
    const expanded_match_id = find_what_under_cursor(e, all_data.rounds, state.scroll_X, options).hovered_match?.id
...
export const install_mouse_events = (
    ...
    root_brackets_el.addEventListener(
        ...
        throttle(e => {
            ...
            const { hovered_match, hovered_team } = find_what_under_cursor(e, all_data.rounds, state.scroll_X, options)
