...
export const install_mouse_events = (
    ...
    matches_scrollable_area.addEventListener(
        ...
        // default: highlight team history
            if (options.use_mobile_layout || !e.target.classList.contains('side-wrapper')) {
    ...
    const scroll_left = e => e.button === 0 && !scrollbar_is_dragged && try_update_scroll_round_index(-1, all_data, store, stable_elements)
    const scroll_right = e => e.button === 0 && !scrollbar_is_dragged && try_update_scroll_round_index(1, all_data, store, stable_elements)
