...
export const install_mouse_events = (
        ...
        throttle(e => {
            ...
            if (options.show_scores_only_on_hover) {
                ...
                if (!hovered_match && state.hovered_match_id) {
                    ...
                    // animate_with_easing(
                    //     easing_value => {
                    //         update_state({ hovered_match_opacity: 1-easing_value })
                    //     },
                    //     300
                    // )
                }
    ...
    if (options.show_scores_only_on_hover) {
        root_brackets_el.addEventListener(
            'mousemove',
            debounce(e => {
                const hovered_match_id = get_hovered_match(e, all_data, state, options)?.id
                if (hovered_match_id === state.hovered_match_id) return
                update_state({
                    hovered_match_id: hovered_match_id,
