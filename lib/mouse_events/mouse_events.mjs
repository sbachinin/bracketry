...
export const install_mouse_events = (
    ...
    root_elements.wrapper.addEventListener(
        ...
        e => {
            ...
            if (e.target === root_elements.main_canvas_el) {
                try_update_hovered_team(e, all_data.rounds, store, options)
    ...
    root_elements.main_canvas_el.addEventListener('mouseup',
        e => {
            // only if left button
            
            // written when listener was attached to root_el, suppose it's not needed any more, but must check:
            // if (!e.target.classList.contains(MAIN_CANVAS_CLASS_NAME)) return
