...
import { start_dragging, try_drag, handle_drag_stop } from './drag.mjs'
...
export const install_mouse_events = (
    ...
    root_brackets_el.addEventListener(
        ...
        e => {
            ...
            try_drag(e, all_data.rounds, store)
        }
    )
    ...
    const canvas_el = root_brackets_el.querySelector(`.${CANVAS_CLASS_NAME}`)
    canvas_el.addEventListener("mousedown", e => start_dragging(e, store));
    canvas_el.addEventListener('mouseup', e => handle_drag_stop(e, store, options))
    canvas_el.addEventListener('mouseleave', e => handle_drag_stop(e, store, options))
}
