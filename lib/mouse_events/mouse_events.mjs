...
const try_select_team = debounce((e, rounds, scroll_X, options, update_state) => update_state({
    highlighted_team_id: find_what_under_cursor(e, rounds, scroll_X, options).hovered_team?.id
}), 70)
...
export const install_mouse_events = (
    ...
    root_brackets_el.addEventListener(
        ...
        e => {
            ...
            options.highlight_team_history_on_hover && try_select_team(
                e, all_data.rounds, state.scroll_X, options, update_state
            )
        }
    )
    ...
    root_brackets_el.addEventListener(
        ...
        e => {
            ...
            if (options.reduce_match_until_clicked) {
                try_update_expanded_match(e, all_data.rounds, state, options, update_state)
            }

            if (options.highlight_team_history_on_click) {
                try_select_team(e, all_data.rounds, state.scroll_X, options, update_state)
            }
        }
    )
