...
export const install_mouse_events = (
    ...
    matches_scrollable_area.addEventListener(
        ...
        e => {
            ...
            if (!e.target.classList.contains('side-body')) {
                store.update_state({ highlighted_contestant_id: null })
                return
            }
            const contestant_id = e.target.closest('.side-wrapper').getAttribute('contestant-id')
            store.update_state({ highlighted_contestant_id: contestant_id ?? null })
        }
    )
