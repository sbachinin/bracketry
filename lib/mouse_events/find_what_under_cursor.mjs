...
export const find_what_under_cursor = (e, all_data, state, options) => {
    ...
    const hovered_round_index = all_data.rounds.findIndex((r, i) => {
        return (
            (state.scroll_X + cursor_position.x) > r.static_left_X + options.match_hor_margin
            && (state.scroll_X + cursor_position.x) < (r.static_left_X + all_data.width_per_round - options.match_hor_margin)
        )
    })
