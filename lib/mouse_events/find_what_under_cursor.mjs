...
export const find_what_under_cursor = (e, rounds, scroll_X, options) => {
    ...
    const hovered_match = hovered_round?.matches.find(m => {
        return Math.abs(e.offsetY - m.center_Y) <= get_match_body_height(options)/1.7
    })

    let hovered_team = undefined
    if (hovered_match && e.offsetY < (hovered_match.center_Y - options.vert_gap_between_opponents / 2)) {
