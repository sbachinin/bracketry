...
export const maybe_start_dragging = (e, rounds, store, options) => {
    ...
    if ((options.reduce_match_until_clicked || options.highlight_team_history_on_click)
        && find_what_under_cursor(e, rounds, store.state.scroll_X, store.state.scroll_Y, options).hovered_match ) return
...
export const maybe_stop_drag = (e, all_content_width, store) => {
    if (!store.state.grabbing) return
