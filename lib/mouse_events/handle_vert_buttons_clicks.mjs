...
export const handle_vert_buttons_clicks = (e, vert_buttons, store, options) => {
    let destination_scroll_Y = store.state.scroll_Y

    if (e.target == vert_buttons.up_button.wrapper
        || vert_buttons.up_button.wrapper.contains(e.target)
    ) {
        destination_scroll_Y -= options.vertical_shift_on_button_clicks
    }
    ...
    destination_scroll_Y = within_range(destination_scroll_Y, 0, 100)

    if (destination_scroll_Y !== store.state.scroll_Y) {
        animate_scroll({
            store,
